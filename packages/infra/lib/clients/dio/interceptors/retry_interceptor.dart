...
///
/// This implementation consolidates functionality previously split between
/// RetryInterceptor and DioRetryInterceptor.
...
class RetryOptions {
  ...
  /// Creates retry options that only retry on non-cancel errors
  /// (compatible with the old dio_retry_interceptor behavior)
  factory RetryOptions.legacyMode() {
    return RetryOptions(
      retryEvaluator: (DioException error) {
  ...
  /// Returns [true] if the response hasn't been cancelled and
  /// is not a bad response.
  ///
  /// This is the default behavior that checks both cancel and bad response errors.
  ...
  factory RetryOptions.fromExtra(
    ...
    final dynamic options = request.extra[extraKey];
    return options is RetryOptions ? options : defaultOptions;
  }
